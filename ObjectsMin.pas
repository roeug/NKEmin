...
const
  ...
  STVStrLongError = 'String length of PAnsiChar exceeds 65527 chars';
...
procedure RaiseTVErrShS(const Msg: string; const S: string); forward;
...
procedure RaiseNotImplemented(const aStr: string); forward;
...
function GetFileSHA1Digest(const FileName: String; var Digest: TSHA1Digest): Integer;
begin
 ShowMessage('TODO Linux GetFileSHA1Digest');
end;
{$ENDIF}
...
function GetFileMixDigest(const FileName: String; var Digest: TMixDigest): Integer;
begin
 ShowMessage('TODO Linux GetFileMixDigest');
end;
{$ENDIF}
...
function TVUniCollection.SearchNext_PChar(PIR: PTVIndexRec; AKey: PAnsiChar;
  var Index: Integer): Boolean;
var
  L,H,I: Integer;
  C: Integer;
...
function TVUniCollection.SearchNext_PChar_I(PIR: PTVIndexRec; AKey: PAnsiChar;
  var Index: Integer): Boolean;
var
  L,H,I: Integer;
  C: Integer;
...
function TVUniCollection.SearchObj_PChar(PIR: PTVIndexRec; AKey: PAnsiChar): Pointer;
var
  I: Integer;
begin
...
function TVUniCollection.SearchObj_PChar_I(PIR: PTVIndexRec; AKey: PAnsiChar): Pointer;
var
  I: Integer;
begin
...
function TVUniCollection.SelectRange_PChar(PIR: PTVIndexRec; LeftBound,
  RightBound: PAnsiChar; var Index: Integer; LeftTerm, RightTerm: Integer): Integer;
var
  L,H,I: Integer;
  C: Integer;
begin
...
function TVUniCollection.StartWith_PChar(PIR: PTVIndexRec; P: PAnsiChar;
...
function TVUniCollection.StartWith_PChar_I(PIR: PTVIndexRec; P: PAnsiChar;
  var Index: Integer): Integer;
var
  L,H,I: Integer;
  C: Integer;
...
begin
  ...
  begin
    ...
    Flag := True;
    L := Length(S);
    SetString(DStr,PAnsiChar(S[1]),L);
    PD := Pointer(DStr[1]);
...
begin
  Clear;
  SetString(S,PAnsiChar(Value),Length(Value));
  while Length(S) > 0 do
...
//  FHandle := Integer(Windows.CreateFile(PWideChar(TmpStr), GENERIC_READ or GENERIC_WRITE,
...
procedure TVStream.CreateFile(const AFileName: String);
begin
 ShowMessage('TODO Linux TVStream.CreateFile');
end;
{$ENDIF}
...
//  FHandle := longint(FMemoryStream);
{$IFDEF CPU32}
  FHandle := longint(FMemoryStream);
{$ELSE}
  FHandle := int64(FMemoryStream);
{$ENDIF}
  if (FHandle = -1) then //INVALID_HANDLE_VALUE then
   RaiseStrmOpenError(AFileName);
  FFileSize := FMemoryStream.Size;
  if FFileSize < 0 then
   RaiseTVErrS(STVStrmInvalidFileSize, AFileName);
  FFileName := AFileName;
  FMode := AMode;
  FDataView := FMemoryStream.Memory;
  FData := FDataView;
  FFileName := AFileName;
  FMode := AMode;
 end;
...
begin
 ...
 begin
  if FData <> nil then
  begin
   //_FormNK.Memo1.Lines.Add('Count='+IntToStr(DWORD(Count)));
   Q_CopyMem(FData, @Buffer, Count);
// 64   Inc(integer(FData), Count);
   FData:= FData + Count;
  end
  else if FileRead(FHandle, Buffer, Count) <> Count then
   ...
   repeat
    ...
    begin
     if Encryption <> peNone then
      RaiseNotImplemented(
       'Encryption not implemented TVStream.Read(var Buffer; Count: Integer)');
     if Validation <> pvNone then
      RaiseNotImplemented(
       'Validation not implemented TVStream.Read(var Buffer; Count: Integer)');
    end;
    P := P^.Next;
   until P = nil;
 end;
  ...
  if P <> nil then
   repeat
    ...
    begin
     if Encryption <> peNone then
      RaiseNotImplemented(
       'Encryption not implemented TVStream.ReadB(var Buffer; Count: Integer)');
     if Validation <> pvNone then
      RaiseNotImplemented(
       'Validation not implemented TVStream.ReadB(var Buffer; Count: Integer)');
    end;
    P := P^.Next;
   until P = nil;
 end;
...
begin
  ...
  begin
    ...
    begin
     ...
     FileWriteCount:= FileWrite(FHandle,  Buffer, Count);
     if FileWriteCount<>Count THEN
...
begin
 ...
 begin
  case Origin of
// 64 3 strings    0: integer(FData) := integer(FDataView) + Offset;    1: Inc(integer(FData), Offset);    2: integer(FData) := integer(FDataView) + FFileSize - Offset;
   0: FData := FDataView + Offset;
   1: FData:= FData+ Offset;
   2: FData := FDataView + FFileSize - Offset;
  end;
  Result := integer(FData) - integer(FDataView);
  if (Result < 0) or (Result > FFileSize) then
   RaiseStrmSeekError(FFileName);
...
procedure TVStream.SetSize(NewSize: Integer);
begin
 ShowMessage('TODO Linux TVStream.SetSize');
end;
{$ENDIF}
...
procedure TVStream.Flush;
begin
 ShowMessage('TODO Linux TVStream.Flush');
end;
{$ENDIF}
...
procedure TVStream.Truncate;
begin
 ShowMessage('TODO Linux TVStream.Truncate');
end;
{$ENDIF}
...
begin
 ...
 begin
  Self.Read(RegID, 2);
  if RegID <> 0 then //1002 0 1001 in 64bit 49151     QQQQQQ
  begin
 ...
 begin
  RaiseNotImplemented('Security<>nil not implemented TVStream.Get(Security: PTVProtection): TVObject;');
  Result := nil;
 end;
end;
...
procedure TVStream.Put(P: TVObject; Security: PTVProtection);
begin
 ShowMessage('TVStream.Put TODO64');
end;
...
begin
  ...
  begin
    StB[0] := Byte(L);
    Q_PStr2ToOemL(Pointer(S),Pointer(@StB[1]),L);
    Write(StB, L+1);
  end else
    Write(L, 1);
end;
...
begin
  ...
  begin
    ...
    begin
      SetString(Result, nil, L);
      Q_PStr2ToAnsiL(Pointer(@StB[1]),Pointer(Result),L);
    end else
      Result := '';
  end else
    Result := '';
...
function TVStream.StrRead: PAnsiChar;
var
  L: Word;
begin
  Read(L, 2);
...
procedure TVStream.StrWrite(P: PAnsiChar);
type
...
begin
  ...
  begin
    ...
    begin
      PStmWord(@StB)^ := Word(L);
      Q_PStr2ToOemL(P,Pointer(@StB[2]),L);
      Write(StB,L+2);
    end else
    begin
      GetMem(Buf,L+2);
      PStmWord(Buf)^ := Word(L);
      Q_PStr2ToOemL(P,Pointer(@Buf[2]),L);
...
begin
  Read(R, 6);
  Result := FloattoCurr(R);  // QQ
end;

procedure TVStream.WriteCurrencyAsReal(V: Currency);
begin
 ShowMessage('TVStream.WriteCurrencyAsReal TODO');
end;
...
begin
 ShowMessage('TVStream.WriteDoubleAsReal TODO');
end;
...
procedure TVStorage.DeleteAt(Index: Integer);
begin
 ShowMessage('TODO Linux TVStorage.DeleteAt');
end;
{$ENDIF}
...
function TVStorage.GetTg(Key: integer; var ATag: integer;
 Security: PTVProtection): TVObject;
var
 I: integer;
begin
 if Find(Key, I) then
...
procedure TVStorage.PutTg(Key: Integer; Item: TVObject; ATag: Integer; Security: PTVProtection);
begin
 ShowMessage('TODO Linux TVStorage.PutTg');
end;
{$ENDIF}
...
procedure TVStorage.Sweep;
begin
 ShowMessage('TODO Linux TVStorage.Sweep');
end;
{$ENDIF}
    ...
    begin
      ...
      begin
        ...
        if Num > 63 then
          Num := 63;
        Info[0] := Byte(Num);
        Q_PStr2ToOemL(Pointer(FInfoStr),Pointer(@Info[1]),Num);
        if Num <> 63 then
...
begin
 if FModified then
  ShowMessage('TODO CPU64 TVStorage.Flush');
end;
 {$ENDIF}
{$ELSE}
procedure TVStorage.Flush(ForcedWrite: Boolean);
begin
 if FModified then
  ShowMessage('TODO Linux TVStorage.Flush');
end;
{$ENDIF}
...
procedure TVRandomFile.Sweep(Shrink: Boolean);
begin
 ShowMessage('TODO Linux TVRandomFile.Sweep');
end;
{$ENDIF}
...
procedure TVRandomNumFile.Sweep(Shrink: Boolean);
begin
 ShowMessage('TODO Linux TVRandomNumFile.Sweep');
end;
{$ENDIF}
...
procedure RaiseTVErrShS(const Msg: string; const S: string);
begin
  raise Exception.
  CreateFmt(Msg,[S]);
end;
...
procedure RaiseNotImplemented(const aStr: string);
begin
  ShowMessage(aStr);
end;
