...
var
 I, Zdiff: integer;
 D, Dmin, sq, A, B, x1, x2, y1, y2: double; //C
begin
...
procedure TChainCAD.Zoom(const Factor: double);
var
 I: integer;
begin
       ...
       for C := 0 to ChainZList.Count - 1 do
        ...
        // aRec
          ...
          fCanvas.TextOut((aRect.Left + aRect.Right -
           ...
           + 1, aState.Name);
          if PrepareToParse((aState.ValuesStr[0]), aStr) then
          begin // Lambda
           if ValEuSilent(aStr, aFloat) then
            if (aFloat > 0) then
            begin
             aStr := T1_2ToStr(Ln2 / aFloat, 4);
...
var
 ...
 L, CosFi, SinFi: double;
 Arrow: array[0..6] of TPoint;
 Xs, Ys: array[0..6] of double;
 aStr: ShortString;
begin
 ...
 begin
  ...
  if (fChainCAD <> nil) then
   if (fChainCAD.Canvas <> nil) then
    with fChainCAD.Canvas do
    begin
     if PrepareToParse(fLink.ValuesStr[0], aStr) then
     begin
      aStr := (Copy(fLink.ValuesStr[0], 1, Pos('*', fLink.ValuesStr[0]) - 1));
      if ValEuSilent(aStr, aFloat) then
      ...
      begin
       ...
       begin
        ...
        TextY := EuLibMin.EuMin(Arrow[0].Y, Arrow[1].Y) + 1;
       end
       else if (IsAlphaLink) then
       begin
        Self.fChainCAD.fCanvas.Font.Style := Self.fChainCAD.fCanvas.Font.Style + [fsBold];
        TextX := EuLibMin.EuMin(Arrow[0].X, Arrow[1].X) - Round(Self.fChainCAD.fDX * CosFi) - 1;
        TextY := EuLibMin.EuMin(Arrow[0].Y, Arrow[1].Y) - Round(Self.fChainCAD.fDY * SinFi);
       end
       else
       begin // ?? ??????
