...
{$ENDIF}
  classes, Controls, messages, forms, Dialogs, graphics, extctrls, menus, comctrls,
 EuLibMin, NuclideClassesMin;
type
 ...
 public
  ...
  function AddLinkByName(const Start: string; const Finish: string;
   ...
   const aDataModule: TObject = nil): TChainLink;
 ...
 protected
  ...
  function GetFluxDecrease(ValuesStrNo: integer): double;
  function GetDecayDecrease: double;
  function GetThermalDecrease: double;
  function GetResonanceDecrease: double;
  function GetFastDecrease: double;
  function GetG_Factor: double;
  function GetStateName: string;
 public
  function GetDecayTypeValue(const aDT: TDecayType = dtNone): double; // dtNone - total == Lambda
  function GetSigmaF(const SpectrumPart: integer): double;
  (*
  ...
  *)
  ...
  property DecayDecrease: double read GetDecayDecrease;
  property ThermalDecrease: double read GetThermalDecrease;
  property ResonanceDecrease: double read GetResonanceDecrease;
  property FastDecrease: double read GetFastDecrease;
  property G_Factor: double read GetG_Factor;
...
var
 i: integer;
 TmpFloat, SigmaAdd, RIadd: double;
begin
 Self.create(aChain);
...
function TChainState.GetDecayDecrease: double;
var
 IsLambda: Boolean;
 aStr, OutStr: ShortString;
 aFloat: double;
...
function TChainState.GetFluxDecrease(ValuesStrNo: integer): double; // qq PrepareToParse tuned
var
 aStr, OutStr: ShortString;
begin
...
function TChainState.GetFastDecrease: double;
begin
 Result:= GetFluxDecrease(3);
end;

function TChainState.GetResonanceDecrease: double;
begin
...
function TChainState.GetThermalDecrease: double;
begin
...
function TChainState.GetSigmaF(const SpectrumPart: integer): double; // 0-thermal, 1-RI, 3-Fast
var
 J: integer;
 aStr, OutStr: ShortString;
begin
...
function TChainState.GetDecayTypeValue(const aDT: TDecayType = dtNone): double;
var
 J, CommentBegin, DecayStrPos: integer;
 aStr, OutStr, aDTstr: ShortString;
 aLambda: double;
...
function TChainState.GetG_Factor: double; // qq PrepareToParse tuned
var
 aStr, OutStr: ShortString;
 I: integer;
begin
